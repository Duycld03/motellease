<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-bold text-slate-900 dark:text-white">Khu trọ được phân công</h1>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Danh sách khu trọ bạn được chủ nhà giao nhiệm vụ quản lý và vận hành</p>
      </div>
    ...
    <div v-else-if="properties.length === 0" class="p-12 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 text-center text-xs text-slate-400">
      ...
      <p class="font-medium text-slate-500 dark:text-slate-400">Bạn chưa được phân công quản lý khu trọ nào.</p>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-2 text-xs text-slate-600 dark:text-slate-400 pt-2 border-t border-slate-100 dark:border-slate-800">
          <div>⚡ Điện: <strong class="text-slate-900 dark:text-white">{{ formatCurrency(p.electricityPrice || 0) }}/kWh</strong></div>
          <div>💧 Nước: <strong class="text-slate-900 dark:text-white">{{ formatCurrency(p.waterPrice || 0) }}/m³</strong></div>
        </div>

        <div class="flex items-center justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
          <NuxtLink :to="`/staff/appointments?boardingHouseId=${p.id}`" class="btn-outline !text-xs !py-1.5 !px-3">
            📅 Lịch xem phòng
          </NuxtLink>
          <NuxtLink :to="`/staff/tasks?boardingHouseId=${p.id}`" class="btn-primary !text-xs !py-1.5 !px-3">
            ⚡ Công việc
          </NuxtLink>
        </div>
      </div>
