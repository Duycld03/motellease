<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 class="text-xl font-bold text-slate-900">{{ $t('nav.properties') }}</h1>
        <p class="text-xs text-slate-500 mt-1">Danh sách các khu trọ bạn sở hữu hoặc được phân công quản lý</p>
      </div>
    ...
    <!-- Empty state -->
    <div v-else-if="items.length === 0" class="py-16 text-center bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
      <div class="w-14 h-14 rounded-2xl bg-primary-50 text-primary-600 flex items-center justify-center mx-auto mb-4">
        <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
        </svg>
      </div>
      <h3 class="text-sm font-bold text-slate-800">Bạn chưa có khu trọ nào</h3>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div
        ...
        :key="house.id"
        class="bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between"
      >
        <div>
          <!-- Thumbnail & Status -->
          <div class="aspect-[16/9] bg-slate-100 relative overflow-hidden flex items-center justify-center text-slate-400">
            <img
          ...
          <div class="p-5 space-y-3">
            ...
            <!-- Stats strip -->
            <div class="grid grid-cols-2 gap-2 p-3 bg-slate-50 rounded-xl border border-slate-100 text-center">
              <div>
                <span class="text-[10px] text-slate-400 block">Tổng số phòng</span>
                <span class="text-xs font-bold text-slate-800">{{ house.roomCount }} phòng</span>
              </div>
              <div>
                <span class="text-[10px] text-slate-400 block">Phòng trống</span>
                <span class="text-xs font-bold text-emerald-600">{{ house.availableRoomCount }} trống</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Footer Actions -->
        <div class="px-5 pb-5 pt-2 flex items-center justify-between gap-2 border-t border-slate-100 mt-2">
          <NuxtLink
          ...
          <NuxtLink
            ...
            class="p-2 text-slate-500 hover:text-slate-900 rounded-lg hover:bg-slate-100 transition-colors"
